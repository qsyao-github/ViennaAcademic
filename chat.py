...
class ChatManager:
    ...
    @staticmethod
    def stream_response(text: str, files: List[str], thread_id: str,
                        multimodal: bool,
                        timestamp: str) -> Generator[str, None, None]:
        """流式处理聊天响应"""
    ...
    def append_search_result(query: str, focus_mode: str, thread_id: str) -> str:
        ...
        chat_app.update_state(
                    {"configurable": {"thread_id": thread_id}}, {"messages": [HumanMessage(f"请搜索{query}"), AIMessage(search_result)]})
