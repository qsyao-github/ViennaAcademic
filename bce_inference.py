...
def update(current_dir: str) -> None:
    knowledgeBase = set(
        ...
        for file in os.listdir(f'{current_dir}/knowledgeBase'))
    retrievers = set(
        os.path.splitext(file)[0]
        for file in os.listdir(f'{current_dir}/retrievers'))
...
def merge_retrievers(current_dir: str) -> ContextualCompressionRetriever:
    ...
    retrievers = [
        FAISS.load_local(retriever_directory,
                         bce_embedding_base,
                         file,
                         distance_strategy=DistanceStrategy.MAX_INNER_PRODUCT,
                         allow_dangerous_deserialization=True) for file in set(
                             os.path.splitext(file)[0]
                             for file in os.listdir(retriever_directory))
    ]
...
def get_response(query: str, current_dir: str) -> str:
    ...
    for document in response:
        ...
        if content and '#' not in content and '\n' not in content and any(char in content for char in
