...
async def get_response(query: str, texts: List[str]) -> List[Document]:
    """使用bce embedding+rerank召回相关性>=0.35的结果
...
def rank_and_filter_results(
    response: List[Document], results: List[Tuple[str, str, str]]
) -> Generator[Tuple[str, str, str], None, None]:
    """对召回结果进行排序、筛选
...
async def select_academic_search_result(
    ...
    response = await get_response(query, texts)
    return rank_and_filter_results(response, results)
