...
pub static VALID_EXTS: Lazy<HashSet<&'static str>> = Lazy::new(|| {
...
pub fn chat_utils_media_handler_create_image_component(py: Python, image_path: &str) -> Py<PyDict> {
    ...
    let ext = match Path::new(&image_path).extension().and_then(|e| e.to_str()) {
        Some(ext) => ext,
        None => return dict.into(),
    };
    ...
    image_url
        .set_item("url", format!("data:image/{};base64,{}", ext, encoded))
        .unwrap();
